...
@foreach($points as $points=>$index)
    <tr>
        <td align="right">{{ $index->no }}&emsp;</td>
        <td> {{ $index->name }}</td>
        <td>      <input type="button" value="編集" onclick="location.href='/point_edit?point_id={{$index->id}}&rogaining_id={{$index->rogaining_id}}&user_id={{$user_id}}'">
        </td>
    </tr>
@endforeach
</table>
